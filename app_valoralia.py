...
.stApp { background: linear-gradient(rgba(255,255,255,0.93), rgba(255,255,255,0.93)), url("https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2070&auto=format&fit=crop"); background-size: cover; background-position: center; background-attachment: fixed; }
...
.stButton>button { background-color: #0f172a; color: #ffffff; border-radius: 4px; border: none; padding: 0.8rem 2rem; font-size: 1.1rem; font-weight: bold; width: 100%; transition: 0.3s; }
...
.info-box { background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1rem; font-size: 0.85rem; color: #4b5563; line-height: 1.6; margin-top: 1rem; }
...
@st.cache_resource
def cargar_paquete():
    return joblib.load(Path(__file__).parent / "valoralia_production.pkl")
...
def cargar_pipeline_visual():
    try:
        ...
        if not ruta_pca.exists():
            return None, "valoralia_pca_transformer.pkl no encontrado en el repositorio"
        pca = joblib.load(ruta_pca)
        modelo_rn = models.resnet50(weights=ResNet50_Weights.DEFAULT)
        ...
        return {"pca": pca, "extractor": extractor, "transformacion": transformacion, "torch": torch}, None
...
pipeline_visual, error_visual = cargar_pipeline_visual()
vision_disponible = pipeline_visual is not None

ZONAS_DISPLAY = {'Alcala_Henares':'Alcala de Henares','Alcobendas':'Alcobendas','Alcorcon':'Alcorcon','Arganda':'Arganda del Rey','Boadilla':'Boadilla del Monte','Carabanchel':'Carabanchel','Centro':'Centro','Chamartin':'Chamartin','Chamberi':'Chamberi','Ciudad_Lineal':'Ciudad Lineal','Colmenar_Viejo':'Colmenar Viejo','Coslada':'Coslada','Fuencarral':'Fuencarral-El Pardo','Fuenlabrada':'Fuenlabrada','Getafe':'Getafe','Las_Rozas':'Las Rozas','Latina':'Latina','Leganes':'Leganes','Majadahonda':'Majadahonda','Moncloa':'Moncloa-Aravaca','Moratalaz':'Moratalaz','Mostoles':'Mostoles','Parla':'Parla','Pinto':'Pinto','Pozuelo':'Pozuelo de Alarcon','Puente_Vallecas':'Puente de Vallecas','Retiro':'Retiro','Rivas':'Rivas-Vaciamadrid','SS_Reyes':'San Sebastian de los Reyes','Salamanca':'Salamanca','San_Blas':'San Blas-Canillejas','Tetuan':'Tetuan','Torrejon':'Torrejon de Ardoz','Tres_Cantos':'Tres Cantos','Usera':'Usera','Vicalvaro':'Vicalvaro','Villa_Vallecas':'Villa de Vallecas','Villaverde':'Villaverde','Villaviciosa_Odon':'Villaviciosa de Odon'}
...
with col_izq:
    ...
    c1,c2 = st.columns(2)
    zona_idx = c1.selectbox("Zona de Madrid", range(len(ZONAS_LABELS)), format_func=lambda i: ZONAS_LABELS[i], index=6)
    zona = ZONAS_KEYS[zona_idx]
    tipo_idx = c2.selectbox("Tipo de inmueble", range(len(TIPOS_DISPLAY)), format_func=lambda i: TIPOS_DISPLAY[i], index=0)
...
with col_der:
    ...
    if vision_disponible:
        st.info("Sube fotografias del interior. La red neuronal ResNet50 analizara la calidad visual (acabados, iluminacion, estado) para ajustar la tasacion.")
    else:
        st.error(f"Vision artificial no disponible: {error_visual}")
    imagenes = st.file_uploader("Fotografias del interior", type=['jpg','jpeg','png'], accept_multiple_files=True)
    if imagenes:
...
if boton:
    datos = {"superficie_m2":float(superficie),"habitaciones":float(habitaciones),"banos":float(banos),"planta":float(planta),"num_imagenes":float(len(imagenes)) if imagenes else 0.0,"codigo_postal":0.0,"ascensor":mapear(ascensor),"terraza":mapear(terraza),"garaje":mapear(garaje),"calefaccion":mapear(calefaccion),"estado_reforma":mapear(estado_reforma),"zona_scraping":zona,"tipo_inmueble":tipo}
    usa_fotos = False
            ...
            try:
                ...
                if len(vectores) > 0:
                    ...
                    for i, col in enumerate(cols_pca):
                        datos[col] = float(componentes[i])
                    st.success(f"ResNet50 ha analizado {len(vectores)} fotografias correctamente.")
            except Exception as e:
                st.error(f"Error procesando fotografias: {type(e).__name__}: {e}")
    if not usa_fotos:
        for col in cols_pca:
    ...
    st.markdown(f'<div class="info-box"><strong>Metodologia:</strong> Modelo hibrido XGBoost entrenado con 7.970 inmuebles reales de Pisos.com (marzo 2026) y 103.438 fotografias procesadas con ResNet50. El intervalo se basa en el MAE historico ({mae:,.0f} EUR). Fuente visual: {fuente}. R2 logaritmico: 0,9146. MAPE: 20,2%</div>', unsafe_allow_html=True)

st.markdown('<div class="footer"><strong>Valoralia Systems</strong> v1.0<br>Maria Luisa Ros Bolea · TFM Master IA y Big Data · CEU San Pablo 2025-2026<br>Datos reales de Pisos.com · 39 zonas de Madrid · Modelo validado con CV 5-fold</div>', unsafe_allow_html=True)
